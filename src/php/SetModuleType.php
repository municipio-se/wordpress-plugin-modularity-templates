...
class SetModuleType {
  ...
  public function set_module_type_scripts() {
    ...
    if (!self::is_edit_module_view($current_screen->id) || !$post) {
      return;
    }

    $module_areas_modules = $this->modularity_editor->getPostModules($post->ID);

    if (empty($module_areas_modules)) {
      return;
    }
    ...
    foreach ($modules as $index => $module) {
      $modules[$index]["usage"] = $this->module_manager->getModuleUsage(
        $module["id"],
      );
    }
    ...
    $labels = apply_filters(
      "modularity-templates/set-module-type/labels",
      $labels,
    );
    ...
    /* enqueue styles */
    wp_enqueue_style(
      "modularity-templates-style",
      plugin_dir_url(__DIR__) . "css/Index.css",
    );
  }

  private static function is_edit_module_view($current_view) {
    $current_id = preg_match("/^(edit-mod-)+\w*/", $current_view)
      ? $current_view
      : "";
    $screen_ids = ["admin_page_modularity-editor", $current_id];
