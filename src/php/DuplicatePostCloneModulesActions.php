...
class DuplicatePostCloneModulesActions
{
    public function __construct()
    {
        ...
        add_action('duplicate_post_post_copy', [$this, 'duplicate_post_modules'], 10, 4);
    }


    public function register_clone_modules_link_action_handler()
    {
        $link_handler = new Link_Handler(new Post_Duplicator(), new Permissions_Helper());

        $link_handler->register_hooks();
    }


    public function duplicate_post_modules($new_post_id, $post, $status, $parent_id)
    {
        // Check if action is valid
        if (!isset($_REQUEST['action']) || empty($_REQUEST['action']) || $_REQUEST['action'] !== 'duplicate_post_clone_modules') {
            return;
        }

        /* We return this function if the post is a module */
        if (preg_match('/(^mod-)/', $post->post_type)) return;
        ...
        foreach ($post_meta_keys as $meta_key) {
            ...
            foreach ($meta_values as $meta_value) {
                ...
                add_post_meta($new_post_id, $meta_key, duplicate_post_wp_slash($meta_value));
            }
        }
    }
    ...
    public function get_modularity_new_meta_value($module_areas)
    {
        ...
        foreach ($module_areas as $modules_index => $modules) {
            ...
            foreach ($modules as $module_index => $module) {
                ...
                $new_module_id = (string) duplicate_post_create_duplicate(get_post($module['postid']));
                ...
                if ($new_module_id) {
                    ...
                    $meta_data[$modules_index][$new_module_index]['columnWidth'] = $module['columnWidth'];
                    $meta_data[$modules_index][$new_module_index]['postid'] = $new_module_id;
                    $meta_data[$modules_index][$new_module_index]['hidden'] = $module['hidden'];
                }
